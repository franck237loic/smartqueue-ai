...
@section('title', $company->name . ' - File d\'attente')
    ...
    <div class="bg-white shadow-sm">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
            <div class="text-center">
                @if($company->logo)
                <img src="{{ $company->logo }}" alt="{{ $company->name }}" class="h-16 mx-auto mb-4">
                @endif
            ...
            <div class="bg-white rounded-xl shadow-lg overflow-hidden hover:shadow-xl transition">
                ...
                <div class="p-6">
                    ...
                    <div class="flex items-center justify-between text-sm text-slate-500 mb-6">
                        <span>{{ $service->estimated_service_time }} min/ticket</span>
                        <span>{{ $service->waiting_tickets_count }} en attente</span>
                    </div>

                    <form method="POST" action="{{ route('company.ticket.take', [$company, $service]) }}" class="space-y-3">
                        @csrf
                        <div>
