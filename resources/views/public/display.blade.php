...
<head>
    ...
    <title>Affichage - {{ $company->name }}</title>
    ...
    <div class="bg-slate-800 border-b border-slate-700">
        <div class="px-8 py-6 flex justify-between items-center">
            ...
            <div class="text-right">
                <p class="text-4xl font-mono">{{ now()->format('H:i') }}</p>
                <p class="text-slate-400">{{ now()->format('d/m/Y') }}</p>
            </div>
        </div>
    </div>
    ...
    <div class="p-8">
        <div class="mb-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                @forelse($calledTickets->take(6) as $ticket)
                <div class="bg-gradient-to-br from-blue-600 to-blue-700 rounded-2xl p-6 calling">
                    <div class="flex justify-between items-start">
                        <div>
                            ...
                            <p class="text-5xl font-bold">{{ $ticket->number }}</p>
                        </div>
                        <div class="text-right">
                            <p class="text-blue-200 text-sm uppercase tracking-wide">Guichet</p>
                            <p class="text-3xl font-bold">{{ $ticket->counter?->name ?? '-' }}</p>
                        </div>
                    </div>
                </div>
                @empty
                ...
                <div class="space-y-4">
                    ...
                    <div class="flex items-center justify-between bg-slate-700 rounded-lg p-4">
                        <div class="flex items-center">
                            <span class="w-10 h-10 bg-blue-600 rounded-lg flex items-center justify-center font-bold">{{ $service->prefix }}</span>
                            <span class="ml-4 font-medium">{{ $service->name }}</span>
                        </div>
                        <span class="text-slate-400">{{ $service->waiting_tickets_count }} en attente</span>
                    </div>
                    @endforeach
                </div>
            </div>
