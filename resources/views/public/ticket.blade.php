            ...
            <div class="mb-6">
                @if($ticket->isCalled())
                    <div class="presence-confirmation bg-green-50 border border-green-200 rounded-lg p-6 mb-6">
                        ...
                        <form method="POST" action="{{ route('client.confirm', ['ticket' => $ticket->id]) }}" class="text-center">
                            @csrf
...
<script>
let lastPosition = {{ $ticket->getPosition() ?? 0 }};
...
async function checkTicketStatus() {
    try {
        const response = await fetch(`/company/{{ $company->id }}/ticket/${ticketId}/status`);
