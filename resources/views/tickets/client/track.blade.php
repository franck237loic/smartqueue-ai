...
<div class="min-h-screen bg-gradient-to-br from-dark-900 to-dark-800 py-8 px-4">
    <div class="max-w-md mx-auto">
        ...
        <div class="bg-gradient-to-br from-brand-600 to-purple-600 rounded-3xl p-8 mb-6 text-center shadow-2xl shadow-brand-500/30">
            ...
            <h1 class="text-6xl font-bold text-white mb-2 ticket-number">{{ $ticket->number }}</h1>
            <p class="text-white/70">{{ $ticket->service->name }}</p>
        </div>
        ...
        <div class="bg-dark-800/50 border border-dark-700 rounded-2xl p-6 mb-4">
            ...
            <div class="flex items-center justify-between">
                ...
                <span id="waitTime" class="text-brand-400 font-medium">
                    @if($status['position'])
                        ~{{ ceil($status['estimated_wait_seconds'] / 60) }} min
                    @else
                        -
                    @endif
                </span>
        ...
        <div class="flex gap-3">
            ...
            <form method="POST" action="{{ route('tickets.cancel', ['company' => $company, 'ticket' => $ticket]) }}" 
        ...
        <div class="mt-8 text-center">
            ...
            <p class="text-dark-600 text-xs">Ticket créé à {{ $ticket->created_at->format('H:i') }}</p>
        </div>
    </div>
</div>
