...
<body class="bg-gray-50 min-h-screen">
            ...
            <div class="text-center">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                    <div class="bg-gray-50 p-4 rounded-lg">
                        ...
                        <p class="text-lg font-bold text-gray-800">{{ $ticket->service->name }}</p>
                    </div>
                    <div class="bg-gray-50 p-4 rounded-lg">
                        <p class="text-gray-500 text-sm">Entreprise</p>
                        <p class="text-lg font-bold text-gray-800">{{ $ticket->company->name }}</p>
                    </div>
                    <div class="bg-gray-50 p-4 rounded-lg">
                        <p class="text-gray-500 text-sm">Statut</p>
                        <p class="text-lg font-bold" id="ticketStatus">
                            {{ $ticket->status === 'CALLED' ? 'APPELÉ' : 'EN ATTENTE' }}
                        </p>
                    </div>
                </div>
                ...
                <div class="mb-8">
                    <div class="flex justify-between text-sm text-gray-600 mb-2">
                        ...
                        <span id="progressText">Position: {{ $ticket->getPosition() }}</span>
                    </div>
                    <div class="w-full bg-gray-200 rounded-full h-4">
                        <div id="progressBar" class="bg-gradient-to-r from-blue-500 to-purple-600 h-4 rounded-full transition-all duration-500" 
                             style="width: {{ max(10, 100 - ($ticket->getPosition() * 10)) }}%"></div>
                    </div>
                </div>
                ...
                <div class="bg-gray-50 p-6 rounded-xl">
                    ...
                    <div id="notificationsList" class="space-y-2 text-left">
                        @if($ticket->progressive_notifications)
                            @foreach($ticket->progressive_notifications as $notification)
                            <div class="bg-white p-3 rounded-lg border-l-4 border-blue-500">
                                <div class="flex items-center justify-between">
                                    <span class="text-sm text-gray-600">
                                        {{ $notification['type'] === 'soon' ? '🕐 Bientôt votre tour' : 
                                           ($notification['type'] === 'next' ? '⚡ Votre tour approche' : '📢 Ticket appelé') }}
                                    </span>
                                    <span class="text-xs text-gray-400">
                                        {{ \Carbon\Carbon::parse($notification['sent_at'])->format('H:i') }}
                                    </span>
                                </div>
                            </div>
                            @endforeach
        ...
        <div class="bg-white rounded-xl shadow-lg p-8 mb-8 slide-animation">
            <div class="text-center">
                ...
                <div class="space-y-4">
                    ...
                    <a href="/ticket/{{ $ticket->id }}/respond?code={{ $ticket->client_response_code }}" 
    ...
    <script>
        ...
        let companyId = {{ $ticket->company->id }};
