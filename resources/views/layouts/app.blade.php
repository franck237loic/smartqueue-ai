    ...
    <div class="flex min-h-screen">
            ...
            <nav class="flex-1 px-4 py-6 space-y-1">
                @if($currentCompany)
                    @if(auth()->user()->isSuperAdmin() || auth()->user()->hasRoleInCompany($currentCompany, 'company_admin'))
                        <a href="{{ route('company.admin.dashboard', $currentCompany) }}" class="flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium {{ request()->routeIs('company.admin.*') ? 'bg-blue-600 text-white' : 'text-slate-300 hover:bg-slate-800 hover:text-white' }}">
                    ...
                    @if(auth()->user()->hasRoleInCompany($currentCompany, 'agent'))
                        <a href="{{ route('company.agent.dashboard', $currentCompany) }}" class="flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium {{ request()->routeIs('company.agent.*') ? 'bg-blue-600 text-white' : 'text-slate-300 hover:bg-slate-800 hover:text-white' }}">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9"/></svg>
                            Guichets Agent
                        </a>
                    @endif
                @endif
                <a href="{{ route('welcome') }}" class="flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium {{ request()->routeIs('welcome') ? 'bg-blue-600 text-white' : 'text-slate-300 hover:bg-slate-800 hover:text-white' }}">
        ...
        <main class="flex-1 md:ml-64">
            ...
            <div class="h-16 bg-white border-b border-gray-200 flex items-center justify-between px-8 sticky top-0 z-40">
                <h2 class="text-lg font-semibold text-text">@yield('title', 'SmartQueue AI')</h2>
    ...
    <nav class="bg-white border-b border-gray-200 sticky top-0 z-50">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between h-16">
                ...
                <div class="flex items-center gap-4">
                    <a href="{{ route('login') }}" class="text-sm font-medium text-gray-600 hover:text-primary transition">Connexion</a>
                    <a href="{{ route('register') }}" class="text-sm font-medium px-4 py-2 rounded-lg bg-primary text-white hover:bg-blue-600 transition">Inscription</a>
                </div>
            </div>
        </div>
    </nav>
