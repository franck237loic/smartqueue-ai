...
<div class="max-w-3xl mx-auto">
    ...
    <div class="mb-6">
        ...
        <p class="text-gray-500 mt-1">ID: {{ $company->id }} • Créée le {{ $company->created_at->format('d/m/Y') }}</p>
    </div>

    <form action="{{ route('super_admin.companies.update', $company) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="bg-gray-800 rounded-xl p-6 card-shadow border border-gray-700">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="address" rows="2" class="w-full px-4 py-2 bg-gray-700 border border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 text-white placeholder-gray-400">{{ old('address', $company->address) }}</textarea>
                </div>
        ...
        <div class="bg-gray-800 rounded-xl p-6 card-shadow border border-gray-700">
            ...
            <div class="grid grid-cols-3 gap-4">
                <div class="text-center p-4 bg-blue-500/20 rounded-lg">
                    <p class="text-2xl font-bold text-blue-400">{{ $company->users()->count() }}</p>
                    <p class="text-sm text-gray-400">Utilisateurs</p>
                </div>
                <div class="text-center p-4 bg-purple-500/20 rounded-lg">
                    <p class="text-2xl font-bold text-purple-400">{{ $company->services()->count() }}</p>
                    <p class="text-sm text-gray-400">Services</p>
                </div>
                <div class="text-center p-4 bg-green-500/20 rounded-lg">
                    <p class="text-2xl font-bold text-green-400">{{ $company->counters()->count() }}</p>
