...
<div class="max-w-2xl mx-auto">
    ...
    <div class="bg-dark-800 rounded-xl card-shadow p-6">
        <form action="{{ route('company.admin.services.store', $company) }}" method="POST" class="space-y-6">
            @csrf
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="number" name="estimated_service_time" value="{{ old('estimated_service_time', 5) }}" required min="1"
                        class="w-full px-4 py-3 bg-dark-900 border border-dark-700 rounded-lg text-white placeholder-dark-600 focus:outline-none focus:border-brand-500 focus:ring-1 focus:ring-brand-500">
                </div>
            </div>
            ...
            <div>
                ...
                <textarea name="description" rows="3"
                    class="w-full px-4 py-3 bg-dark-900 border border-dark-700 rounded-lg text-white placeholder-dark-600 focus:outline-none focus:border-brand-500 focus:ring-1 focus:ring-brand-500">{{ old('description') }}</textarea>
            </div>

            <div class="grid grid-cols-2 gap-4">
                <div>
                    <label class="block text-sm font-medium text-dark-600 mb-2">Timeout absence (min) *</label>
                    <input type="number" name="missed_timeout" value="{{ old('missed_timeout', 5) }}" required min="1"
                        class="w-full px-4 py-3 bg-dark-900 border border-dark-700 rounded-lg text-white placeholder-dark-600 focus:outline-none focus:border-brand-500 focus:ring-1 focus:ring-brand-500">
                </div>
                <div>
                    <label class="block text-sm font-medium text-dark-600 mb-2">Max tickets/jour</label>
                    <input type="number" name="max_daily_tickets" value="{{ old('max_daily_tickets', 100) }}" min="1"
                        class="w-full px-4 py-3 bg-dark-900 border border-dark-700 rounded-lg text-white placeholder-dark-600 focus:outline-none focus:border-brand-500 focus:ring-1 focus:ring-brand-500">
                </div>
            </div>
