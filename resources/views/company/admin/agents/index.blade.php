...
<div class="space-y-6">
    ...
    <div class="flex justify-between items-center">
        ...
        <a href="{{ route('company.admin.agents.create', $company) }}" class="px-4 py-2 bg-brand-600 text-white rounded-lg hover:bg-brand-700 text-sm font-medium shadow-lg shadow-brand-600/30 flex items-center gap-2">
        ...
        <table class="w-full">
            ...
            <tbody class="divide-y divide-dark-700">
                ...
                <tr class="hover:bg-dark-700/30 transition">
                    ...
                    <td class="px-6 py-4">
                        <span class="px-3 py-1 rounded-full text-xs font-medium {{ $agent->pivot->role === 'company_admin' ? 'bg-brand-500/20 text-brand-500' : 'bg-dark-700 text-dark-600' }}">
                            {{ $agent->pivot->role === 'company_admin' ? 'Admin' : 'Agent' }}
                        </span>
                    </td>
                    <td class="px-6 py-4">
                    ...
                    <td class="px-6 py-4 text-right">
                        <form action="{{ route('company.admin.agents.destroy', [$company, $agent]) }}" method="POST" class="inline" onsubmit="return confirm('Retirer cet agent de l\'entreprise ?')">
                            @csrf
                            @method('DELETE')
                ...
                <tr>
                    <td colspan="4" class="px-6 py-8 text-center text-dark-600">
                        Aucun agent. <a href="{{ route('company.admin.agents.create', $company) }}" class="text-brand-500 hover:text-brand-400">Créer un agent</a>
                    </td>
                </tr>
                @endforelse
            </tbody>
