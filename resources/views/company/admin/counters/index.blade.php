...
<div class="space-y-6">
    ...
    <div class="flex justify-between items-center">
        ...
        <a href="{{ route('company.admin.counters.create', $company) }}" class="px-4 py-2 bg-brand-600 text-white rounded-lg hover:bg-brand-700 text-sm font-medium shadow-lg shadow-brand-600/30 flex items-center gap-2">
            ...
            <tbody class="divide-y divide-dark-700">
                ...
                <tr class="hover:bg-dark-700/30 transition">
                    <td class="px-6 py-4">
                        <div class="flex items-center">
                            ...
                            <div class="ml-3">
                                <p class="font-medium text-white">{{ $counter->name }}</p>
                                @if($counter->number)
                                    <p class="text-xs text-dark-600">N° {{ $counter->number }}</p>
                                @endif
                            </div>
                        </div>
                    </td>
                    ...
                    <td class="px-6 py-4 text-right">
                        <div class="flex items-center justify-end gap-2">
                            <a href="{{ route('company.admin.counters.edit', [$company, $counter]) }}" class="p-2 text-dark-600 hover:text-brand-500 transition">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                                </svg>
                            </a>
                            <form action="{{ route('company.admin.counters.destroy', [$company, $counter]) }}" method="POST" class="inline" onsubmit="return confirm('Supprimer ce guichet ?')">
                                @csrf
                                @method('DELETE')
                ...
                <tr>
                    <td colspan="5" class="px-6 py-8 text-center text-dark-600">
                        Aucun guichet. <a href="{{ route('company.admin.counters.create', $company) }}" class="text-brand-500 hover:text-brand-400">Créer un guichet</a>
                    </td>
                </tr>
                @endforelse
            </tbody>
