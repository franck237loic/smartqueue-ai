...
@section('title', 'Modifier Guichet')
...
<div class="max-w-2xl mx-auto">
    ...
    <div class="mb-6">
        ...
        <p class="text-dark-600 mt-1">{{ $counter->name }}</p>
    </div>

    <!-- Form -->
    <div class="bg-dark-800 rounded-xl card-shadow p-6">
        <form action="{{ route('company.admin.counters.update', [$company, $counter]) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name', $counter->name) }}" required
            ...
            <div>
                ...
                <input type="text" name="location" value="{{ old('location', $counter->location) }}"
                    class="w-full px-4 py-3 bg-dark-900 border border-dark-700 rounded-lg text-white placeholder-dark-600 focus:outline-none focus:border-brand-500 focus:ring-1 focus:ring-brand-500">
            </div>

            <div class="flex items-center">
                <input type="checkbox" name="is_active" value="1" id="is_active" {{ old('is_active', $counter->is_active) ? 'checked' : '' }}
