...
<div class="min-h-screen bg-[#F8FAFC]" style="font-family: 'Inter', sans-serif;">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between items-center h-16">
                <div class="flex items-center">
                    ...
                    <div class="ml-3">
                        <h1 class="font-bold text-slate-900">{{ $company->name }}</h1>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div class="bg-white rounded-2xl p-6 shadow-sm border border-slate-100">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-2xl font-bold text-slate-900">{{ $myTicketsToday ?? 0 }}</p>
                    </div>
            ...
            <div class="bg-white rounded-2xl p-6 shadow-sm border border-slate-100">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-2xl font-bold text-slate-900">{{ $myCounters->where('status', 'open')->count() }}</p>
                    </div>
                ...
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-2xl font-bold text-orange-600">
                            {{ $services->sum('waiting_tickets_count') }}
                        </p>
                    </div>
            ...
            <div class="bg-white rounded-2xl p-6 shadow-sm border border-slate-100">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-2xl font-bold text-slate-900">{{ $avgServiceTime ? round($avgServiceTime / 60) . 'm' : 'N/A' }}</p>
                    </div>
            ...
            <div class="bg-white rounded-2xl p-6 shadow-sm border border-slate-100">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-2xl font-bold text-red-600">{{ $missedTicketsToday }}</p>
                    </div>
            ...
            <div class="bg-white rounded-2xl p-6 shadow-sm border border-slate-100">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-2xl font-bold text-green-600">{{ $serviceRateToday }}%</p>
                    </div>
                    <div class="w-12 h-12 bg-green-50 rounded-xl flex items-center justify-center">
                        <svg class="w-6 h-6 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path>
        ...
        <div class="bg-gradient-to-r from-blue-500 to-blue-600 rounded-2xl p-6 text-white mb-8">
            ...
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-3xl font-bold">{{ $currentTicket->number }}</p>
                    <p class="text-blue-100">{{ $currentTicket->service->name }}</p>
                    <p class="text-blue-100">Guichet {{ $currentTicket->counter->number }}</p>
                </div>
                <div class="text-right">
                    <p class="text-sm text-blue-100">Appelé à</p>
                    <p class="text-lg font-semibold">{{ $currentTicket->called_at->format('H:i') }}</p>
                </div>
            </div>
        </div>
        @endif
        ...
        <div class="flex justify-between items-center mb-6">
            ...
            <a href="{{ route('company.agent.history', $company) }}" class="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition-colors flex items-center">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
            ...
            <div class="bg-white rounded-2xl shadow-sm border {{ $counter->isOpen() ? 'border-green-200' : 'border-red-200' }} overflow-hidden hover:shadow-md transition-shadow">
                <div class="px-6 py-4 {{ $counter->isOpen() ? 'bg-green-50' : 'bg-red-50' }}">
                    <div class="flex justify-between items-center">
                        <div class="flex items-center">
                            <div class="w-10 h-10 {{ $counter->isOpen() ? 'bg-green-500' : 'bg-red-500' }} rounded-xl flex items-center justify-center text-white font-bold">
                                {{ $counter->number }}
                            </div>
                            <div class="ml-3">
                                <h3 class="font-semibold text-slate-900">{{ $counter->name }}</h3>
                                <p class="text-xs text-slate-500">{{ $counter->service?->name ?? 'Sans service' }}</p>
                            </div>
                        </div>
                ...
                <div class="p-6">
                    ...
                    <div class="mb-4 p-4 bg-blue-50 rounded-xl">
                        ...
                        <p class="text-2xl font-bold text-blue-700">{{ $counter->currentTicket()->number }}</p>
                    </div>
                    @else
                    ...
                    <div class="flex space-x-2">
                        <a href="{{ route('company.agent.counter', [$company->id, $counter->id]) }}" class="flex-1 px-4 py-2 bg-[#2563EB] text-white rounded-lg hover:bg-blue-700 transition text-center text-sm font-medium">
                            Accéder
                        </a>
            ...
            <div class="divide-y divide-slate-100">
                ...
                <div class="px-6 py-4 flex items-center justify-between hover:bg-slate-50 transition-colors">
                    ...
                    <div class="flex items-center space-x-4">
                        ...
                        <a href="{{ route('company.agent.service', [$company, $service]) }}" class="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition text-sm font-medium">
                            Accéder
                        </a>
                    </div>
                </div>
