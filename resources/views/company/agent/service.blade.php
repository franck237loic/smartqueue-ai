...
@section('title', $service->name . ' - ' . $company->name)
...
<div class="min-h-screen bg-[#F8FAFC]" style="font-family: 'Inter', sans-serif;">
            ...
            <div class="flex justify-between items-center h-16">
                <div class="flex items-center">
                    ...
                    <div class="ml-3">
                        ...
                        <p class="text-xs text-slate-400">{{ $company->name }} Service</p>
                    </div>
                </div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div class="bg-white rounded-2xl p-6 shadow-sm border border-slate-100">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-2xl font-bold text-slate-900">{{ $waitingTickets->count() }}</p>
                    </div>
            ...
            <div class="bg-white rounded-2xl p-6 shadow-sm border border-slate-100">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-2xl font-bold text-slate-900">{{ $service->estimated_service_time }} min</p>
                    </div>
            ...
            <div class="bg-white rounded-2xl p-6 shadow-sm border border-slate-100">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-2xl font-bold text-slate-900">{{ $serviceCounters->count() }}</p>
                    </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
            @forelse($serviceCounters as $counter)
            <div class="bg-white rounded-2xl shadow-sm border {{ $counter->isOpen() ? 'border-green-200' : 'border-red-200' }} overflow-hidden hover:shadow-md transition-shadow">
                    ...
                    <div class="flex justify-between items-center">
                        <div class="flex items-center">
                            ...
                            <div class="ml-3">
                                ...
                                <p class="text-xs text-slate-500">{{ $counter->isOpen() ? 'Ouvert' : 'Fermé' }}</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="p-6">
                    <div class="flex space-x-2">
                        ...
                        @if($counter->isOpen())
                        <form method="POST" action="{{ route('company.agent.counter.close', [$company->id, $counter->id]) }}" class="flex-1">
                            @csrf
                        ...
                        @else
                        <form method="POST" action="{{ route('company.agent.counter.open', [$company->id, $counter->id]) }}" class="flex-1">
                            @csrf
            ...
            <div class="divide-y divide-slate-100">
                ...
                <div class="px-6 py-4 flex items-center justify-between hover:bg-slate-50 transition-colors">
                    <div class="flex items-center">
                        ...
                        <div class="ml-4">
                            <p class="font-medium text-slate-900">Ticket {{ $ticket->number }}</p>
                            <p class="text-sm text-slate-500">{{ $ticket->created_at->format('H:i') }}</p>
                        </div>
                    </div>
                    <div class="flex items-center space-x-4">
                        ...
                        <form method="POST" action="{{ route('company.agent.ticket.call', [$company->id, $ticket->id]) }}" class="inline">
                            @csrf
                        ...
                        <form method="POST" action="{{ route('company.agent.ticket.serve', [$company->id, $ticket->id]) }}" class="inline">
                            @csrf
