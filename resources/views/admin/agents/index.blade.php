...
<div class="space-y-8 animate-fade-in">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        ...
        <a href="{{ route('admin.agents.create') }}" class="py-2 px-4 sm:py-3 sm:px-6 rounded-xl gradient-primary text-white font-medium hover:opacity-90 transition inline-flex items-center gap-2 text-sm sm:text-base">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
            ...
            <div class="bg-white rounded-2xl card-shadow p-4 sm:p-6">
                <div class="flex items-start justify-between mb-3 sm:mb-4">
                    <div class="flex items-center gap-3 sm:gap-4">
                        ...
                        <div class="min-w-0 flex-1">
                            <p class="font-semibold text-text text-base sm:text-lg truncate">{{ $agent->name }}</p>
                ...
                <div class="space-y-2 sm:space-y-3 pt-3 sm:pt-4 border-t border-gray-100">
                    <div class="flex justify-between items-center">
                        ...
                        <span class="font-semibold text-text text-sm sm:text-base">{{ $agent->tickets()->where('status', 'served')->count() }}</span>
                    </div>
                    <div class="flex justify-between items-center">
                        <span class="text-gray-500 text-xs sm:text-sm">Servis aujourd'hui</span>
                        <span class="font-semibold text-success text-sm sm:text-base">{{ $agent->tickets()->where('status', 'served')->whereDate('served_at', today())->count() }}</span>
                    </div>
                    <div class="flex justify-between items-center">
                        <span class="text-gray-500 text-xs sm:text-sm">En appel</span>
                        <span class="font-semibold text-primary text-sm sm:text-base">{{ $agent->tickets()->where('status', 'called')->count() }}</span>
                    </div>
                </div>
        ...
        @empty
            <div class="col-span-full text-center py-8 sm:py-12 bg-white rounded-2xl card-shadow">
                ...
                <a href="{{ route('admin.agents.create') }}" class="text-primary hover:underline font-medium text-sm sm:text-base">Ajouter un agent</a>
            </div>
        @endforelse
    </div>
</div>
