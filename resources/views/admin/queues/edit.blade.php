...
@section('title', 'Modifier File - SmartQueue AI')
...
<div class="max-w-2xl mx-auto animate-fade-in">
    ...
    <div class="bg-white rounded-2xl card-shadow p-8">
        ...
        <form method="POST" action="{{ route('admin.queues.update', $queue) }}" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="3"
                    class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition">{{ old('description', $queue->description) }}</textarea>
            </div>
            ...
            <div>
                ...
                <select id="status" name="status" required
                    ...
                    <option value="paused" {{ $queue->status === 'paused' ? 'selected' : '' }}>En pause</option>
                    <option value="closed" {{ $queue->status === 'closed' ? 'selected' : '' }}>Fermée</option>
                </select>
            </div>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" id="prefix" name="prefix" value="{{ old('prefix', $queue->prefix) }}" required maxlength="5"
                ...
                <div>
                    ...
                    <input type="number" id="estimated_service_time" name="estimated_service_time" value="{{ old('estimated_service_time', $queue->estimated_service_time) }}" required min="1"
                        class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition">
                </div>
            </div>
            ...
            <div>
                ...
                <input type="number" id="missed_timeout" name="missed_timeout" value="{{ old('missed_timeout', $queue->missed_timeout) }}" required min="1"
                    class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition">
            </div>
